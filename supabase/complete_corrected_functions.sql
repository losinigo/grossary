-- ============================================================
-- COMPLETE CORRECTED Enhanced Price Estimation Functions
-- Run this in Supabase SQL Editor to restore all three modes
-- ============================================================
  ...
  selected_prices AS (
    SELECT DISTINCT ON (np.product_id)
      ...
      CASE 
        ...
        ELSE np.price_per_unit -- specific_store
      ...
      -- CORRECTED: Only mark as best deal if we're in optimized mode AND showing the actual minimum
      (estimation_mode = 'optimized' AND np.price_per_unit = ps.min_price_nearby) as is_best_deal
    FROM nearby_prices np
    ...
    WHERE 
      CASE 
        ...
        ELSE TRUE -- near_me can show any store
      END
    ORDER BY np.product_id, np.price_per_unit ASC, np.created_at DESC
  )
  SELECT 
    li.item_id,
    li.product_id,
...
-- Function to get shopping list summary with different modes (RESTORED)
